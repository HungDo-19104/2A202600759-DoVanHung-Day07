...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        if not text:
            return []
        sentence_endings = re.split(r'(?<=[.!?])\s+', text.strip())
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for piece in pieces:
            ...
            if len(candidate) <= self.chunk_size:
                ...
                if buffer:
                    result.extend(self._split(buffer, next_seps))
                else:
        ...
        merged = []
        for r in result:
            if merged and len(merged[-1]) + len(r) <= self.chunk_size:
                merged[-1] = merged[-1] + sep + r
            else:
                merged.append(r)
        return merged
